...
- **要約**: 設定で2モードを切替（`config.yaml` の `summary.enabled`）。
  - `true` … Claude Messages API で日本語要約。デフォルト `claude-sonnet-4-6`（品質重視）。安く回すなら `claude-haiku-4-5-20251001`。**要 `ANTHROPIC_API_KEY`・トークン課金あり**。
  - `false` … APIを一切呼ばず、アブスト原文をそのまま掲載。**API課金ゼロ・APIキー不要**。
...
- `summary.enabled` … **要約モードの切替**。`true`=Claude APIで要約 / `false`=アブスト原文をそのまま掲載（API不要）
- `summary.model` … 要約モデル（`claude-sonnet-4-6` / `claude-haiku-4-5-20251001`）※`enabled: true`時のみ
- `summary.abstract_max_chars` … 要約なし時にアブストを切る文字数（`null`=切らない）
